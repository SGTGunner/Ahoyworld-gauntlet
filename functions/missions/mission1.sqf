...
diag_log format ["mission1 Start %1",_missionCounter];
...
diag_log format ["mission1 2 %1,%2",_selectedLocation,_missionName];
...
_winTrigger setTriggerStatements ["this","missionFinish = true;publicVariableServer 'missionFinish';",""];
...
_TriggerPFH = {
	if ((!isNil "missionFinish") && {missionFinish}) then {
		...
		[(_missionCounter+1),false] call AW_fnc_missionSelection;
